...
void video_refresh_timer(void *userdata) {
    VideoState *is = (VideoState *)userdata;
    VideoPicture *vp;
    double actual_delay, delay, syn_threshold, ref_clock, diff;
    ...
    if (is->video_st) {
        if (is->pictq_size == 0) {
            ...
            fprintf(stderr, "delay %f, pts %f, ref_clock %f, diff %f\n", delay, vp->pts, ref_clock, diff);

            // skip or repeat the frame. Take delay to into account
            // FFPlay steill doesen't "know if this the best guess
